...
interface Row { id: number; from_id: number; to_id: number; label: string | null; }
...
export class TrustEdgeRepository {
  ...
  upsert(fromId: Id, toId: Id, label?: string): void {
    this.db.run(
      ...
      [fromId, toId, label ?? null]
    );
  }

  all(): TrustEdge[] {
    return this.db.all<Row>('SELECT * FROM trust_edges').map(TrustEdgeRepository.map);
  }
}
