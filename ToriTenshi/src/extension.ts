...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  logger.info('Activating ToriTenshi…');
  ...
  logger.info(`ToriTenshi activated (AI: ${config.llmMode}, Obsidian: ${obsidian.isConfigured() ? 'on' : 'off'}, storage: ${config.storageBackend}, grammars: ${config.grammarsPath || 'auto'}).`);
}
