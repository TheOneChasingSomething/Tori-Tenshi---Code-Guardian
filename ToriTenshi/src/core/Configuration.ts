...
export class Configuration {
  ...
  /** SQL engine used for persistence. */
  get storageBackend(): 'better-sqlite3' | 'sql.js' {
    return this.cfg().get<'better-sqlite3' | 'sql.js'>('storage.backend', 'better-sqlite3');
  }

  /** Optional explicit database file path (empty = default in global storage). */
  get storageFile(): string {
    return this.cfg().get<string>('storage.file', '');
  }
